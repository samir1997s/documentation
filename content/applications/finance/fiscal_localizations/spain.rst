...
The following modules related to the Spanish localization are available:
...
.. note::
   ...
   be manually :doc:`installed <../../general/apps_modules>`.
...
The Spanish localization package provides the following key features to ensure compliance with
...
.. note::
   ...
   - By default, new Odoo Online databases are set up with the :guilabel:`Spain - SMEs (2008)`
...
.. tip::
   To export the :guilabel:`VAT records books (XLSX)` (Libros de IVA), click :icon:`fa-book`
...
**Veri*Factu** is an Verifiable Invoice Issuance System used by the Spanish Tax Agency. It is
mandatory for most taxpayers in Spain, except for those who use the SII system or are under a
regional tax regime (i.e., TicketBai).
...
.. note::
   ...
   - :guilabel:`Country` and :guilabel:`Tax ID` fields on the :doc:`company record
     </applications/general/companies>` are filled in.
